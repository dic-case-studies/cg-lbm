...
class LBMSnapshotTest(absltest.TestCase):
    ...
    def test_compute_phi_grad(self):
        ...
        dst_phase_field = pd.read_csv(input_dst_phase_field_path)["dst_phase_field"].to_numpy().reshape(system.LX, system.LY, system.NL).transpose(2, 0, 1)
        expected = pd.read_csv(expected_path)[["phi_grad_x", "phi_grad_y"]].to_numpy().reshape(system.LX, system.LY, 2)
    ...
    def test_compute_density_velocity_pressure(self):
        ...
        phase_field = jnp.array(pd.read_csv(input_phase_field_path)["phase_field"].to_numpy()).reshape(system.LX, system.LY)
        phi_grad = jnp.array(pd.read_csv(input_phi_grad_path)[["phi_grad_x", "phi_grad_y"]].to_numpy()).reshape(system.LX, system.LY, 2)
        total_force = jnp.array(pd.read_csv(input_total_force_path)[["total_force_x", "total_force_y"]].to_numpy()).reshape(system.LX, system.LY, 2)
        rho = jnp.array(pd.read_csv(input_2d_path)["rho"].to_numpy()).reshape(system.LX, system.LY)
        u = jnp.array(pd.read_csv(input_2d_path)[["u_x","u_y"]].to_numpy()).reshape(system.LX, system.LY, 2)
        pressure = jnp.array(pd.read_csv(input_2d_path)["pressure"].to_numpy()).reshape(system.LX, system.LY)
        obs = jnp.array(pd.read_csv(input_obstacle_path)["obs"].to_numpy(dtype=bool)).reshape(system.LX, system.LY) 
        N = jnp.array(pd.read_csv(input_3d_path)["N"].to_numpy()).reshape(system.LX, system.LY, system.NL).transpose(2, 0, 1)
        
        expected_rho = pd.read_csv(expected_path)["rho"].to_numpy().reshape(system.LX, system.LY)
        expected_u = pd.read_csv(expected_path)[["u_x","u_y"]].to_numpy().reshape(system.LX, system.LY, 2)
        expected_pressure = pd.read_csv(expected_path)["pressure"].to_numpy().reshape(system.LX, system.LY)
        expected_interface_force = pd.read_csv(expected_path)[["interface_force_x","interface_force_y"]].to_numpy().reshape(system.LX, system.LY, 2)

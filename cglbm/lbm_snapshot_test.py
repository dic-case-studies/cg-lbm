...
class LBMSnapshotTest(absltest.TestCase):
    ...
    def test_grid_eq_dist(self):
        ...
        actual = grid_eq_dist(system.cXYs, system.weights,
                              system.phi_weights, pressure, u)

        neq_path = epath.resource_path("cglbm") / f'test-data/eq_dist_output.csv'
        expected_neq = pd.read_csv(neq_path)["neq"].to_numpy()
        expected = expected_neq.reshape(
    ...
    def test_compute_phi_grad(self):
        ...
        phase_field = pd.read_csv(input_path)["phase_field"].to_numpy().reshape(system.LX, system.LY)
        ...
        print("\nNumber of different elements: " , len(np.argwhere(np.isclose(expected, actual))))
        self.assertTrue(np.allclose(actual, expected))
